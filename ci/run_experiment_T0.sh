...
if [ ! -z "$IMAGE" ]; then
    ...
    EXP="t0-harmonic-progression-sum"
    ...
    pushd /data/cdslab/$USERNAME
        ...
        # run experiments
        $CDS_TOOL run --measure --image $USERNAME -r 5 -c $NUMBER_CPU -i ./cds-root/mopp/$EXP/judge.in -o ./cds-root/mopp/$EXP/judge.out mopp-$EXP | tee $EXP.$BUILD_TAG.log
